...
#define LC3_DECODER_MULTI_STACK_SIZE (4192)
#define LC3_DECODER_THREAD_PRIORITY  (4)
...
K_THREAD_STACK_DEFINE(lc3_dec_thread_stack, LC3_DECODER_MULTI_STACK_SIZE);
DATA_FIFO_DEFINE(lc3_dec_fifo_tx, LC3_DECODER_MSG_QUEUE_SIZE,
		 sizeof(struct audio_module_message));
DATA_FIFO_DEFINE(lc3_dec_fifo_rx, LC3_DECODER_MSG_QUEUE_SIZE,
		 sizeof(struct audio_module_message));
	...
	case SW_CODEC_LC3: {
		...
		if (bad_frame && IS_ENABLED(CONFIG_SW_CODEC_OVERRIDE_PLC)) {
			for (int i = 0; i < m_config.decoder.num_ch; i++) {
				memset(decoded_data_mono[i], 0, PCM_NUM_BYTES_MONO);
			}
			...
			audio_data_tx.meta.bits_per_sample = 16;
			audio_data_tx.meta.carried_bits_pr_sample = 16;
			audio_data_tx.meta.locations = ~(0xFF << m_config.decoder.channel_mode);

			audio_data_rx.data = (void *)decoded_data_mono;
			audio_data_rx.data_size = PCM_NUM_BYTES_MONO;
			...
			audio_data_rx.meta.bits_per_sample = 16;
			audio_data_rx.meta.carried_bits_pr_sample = 16;
			audio_data_rx.meta.locations = ~(0xFF << m_config.decoder.channel_mode);

			ret = audio_module_data_tx_rx(&handle[LC3_MODULE_ID_DECODER], &handle[LC3_MODULE_ID_DECODER],
...
int sw_codec_init(struct sw_codec_config sw_codec_cfg)
{
	...
	case SW_CODEC_LC3: {
		...
		if (sw_codec_cfg.decoder.enabled) {
			struct audio_module_parameters decoder_param;
			...
			memset(&handle[LC3_MODULE_ID_DECODER], 0, sizeof(struct audio_module_handle));
			...
			decoder_config.interleaved = true;
			decoder_config.locations = ~(0xFF << sw_codec_cfg.decoder.num_ch);
			decoder_config.bitrate_bps = CONFIG_LC3_BITRATE;

			data_fifo_init(&lc3_dec_fifo_rx);
			data_fifo_init(&lc3_dec_fifo_tx);

			ret = k_mem_slab_init(&audio_data_slab, &audio_data_memory[0],
				      PCM_NUM_BYTES_MONO * sw_codec_cfg.decoder.num_ch, LC3_DATA_OBJECTS_NUM);
			if (ret) {
				LOG_ERR("Failed to allocate the data slab: %d", ret);
				return ret;
			}

			decoder_param.description = lc3_decoder_description;
			decoder_param.thread.stack_size = CONFIG_LC3_DECODER_STACK_SIZE;
			decoder_param.thread.priority = CONFIG_LC3_DECODER_THREAD_PRIO;
			decoder_param.thread.data_slab = &audio_data_slab;
			decoder_param.thread.data_size = PCM_NUM_BYTES_MONO * sw_codec_cfg.decoder.num_ch;
			decoder_param.thread.stack = lc3_dec_thread_stack,
			decoder_param.thread.msg_rx = &lc3_dec_fifo_rx;
			decoder_param.thread.msg_tx = &lc3_dec_fifo_rx;

			ret = audio_module_open(
				&decoder_param, (struct audio_module_configuration *)&decoder_config,
				"LC3 Decoder",
