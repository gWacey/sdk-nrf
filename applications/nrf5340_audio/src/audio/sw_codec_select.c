				   ...
				   char **data_mem, size_t data_size, size_t data_num)
{
	...
	handle_size = amod_query_resource(parameters, mod_config);
	if (handle_size < 0) {
		LOG_DBG("Querry resource FAILED , ret %d", handle_size);
		return -1;
	}
	...
	*handle = (struct amod_handle *)k_malloc(handle_size);
	...
	LOG_DBG("Memory allocation completed");
...
int sw_codec_uninit(struct sw_codec_config sw_codec_cfg)
{
	...
	case SW_CODEC_LC3:
		...
		if (sw_codec_cfg.decoder.enabled) {
			...
			dynamic_memory_free((struct amod_handle **)&sw_codec_cfg.lc3_dec1_hdl,
					    &sw_codec_cfg.dec1_in_msg_mem,
					    &sw_codec_cfg.dec1_out_msg_mem,
					    &sw_codec_cfg.dec1_data_mem);
#else
