...
struct btn_units_cfg {
	uint32_t num_buttons;
	struct btn_cfg_data *button;
};
...
#define BUTTON_GPIO_DT_SPEC(button_node_id)                                                        \
	static const struct gpio_dt_spec button_dt_gpio_spec_##button_node_id[] = {                \
		DT_FOREACH_PROP_ELEM_SEP(button_node_id, gpios, GPIO_DT_SPEC_GET_BY_IDX, (,)),    \
	};

#define BUTTON_GPIO(button_node_id)                                                                \
	{                                                                                          \
		.name = STRINGIFY(BUTTON_##button_node_id),                                        \
				  .gpio = button_dt_gpio_spec_##button_node_id,                    \
		},

DT_FOREACH_CHILD(BUTTON, BUTTON_GPIO_DT_SPEC)

static struct btn_cfg_data btn_config_data[] = {DT_FOREACH_CHILD(BUTTON, BUTTON_GPIO)};

static const struct btn_units_cfg btn_config = {.num_buttons = ARRAY_SIZE(btn_config_data),
						.button = btn_config_data};
...
static int pin_to_btn_idx(uint8_t btn_pin, uint32_t *pin_idx)
{
	for (uint8_t i = 0; i < btn_config.num_buttons; i++) {
		if (btn_pin == btn_config.button[i].gpio->pin) {
			*pin_idx = i;
			return 0;
		}
	}
...
static void button_isr(const struct device *port, struct gpio_callback *cb, uint32_t pin_msk)
{
	...
	LOG_DBG("Pushed button idx: %d pin: %d name: %s", btn_idx, btn_pin,
		btn_config.button[btn_idx].name);
...
int button_pressed(gpio_pin_t button_pin, bool *button_pressed)
{
	...
	if (!device_is_ready(btn_config.button[btn_idx].gpio->port)) {
		return -EINVAL;
	}

	ret = gpio_pin_get(btn_config.button[btn_idx].gpio->port, button_pin);
	switch (ret) {
	case 0:
...
int button_handler_init(void)
{
	...
	for (size_t i = 0; i < btn_config.num_buttons; i++) {
		struct btn_cfg_data *button = &btn_config.button[i];
...
static int cmd_print_all_btns(const struct shell *shell, size_t argc, char **argv)
{
	...
	for (uint8_t i = 0; i < btn_config.num_buttons; i++) {
		shell_print(shell, "Id %d: pin: %d %s", i, btn_config.button[i].gpio->pin,
			    btn_config.button[i].name);
	}
...
static int cmd_push_btn(const struct shell *shell, size_t argc, char **argv)
{
	...
	msg.button_pin = btn_config.button[btn_idx].gpio->pin;
	msg.button_action = BUTTON_PRESS;
	...
	shell_print(shell, "Pushed button idx: %d pin: %d : %s", btn_idx,
		    btn_config.button[btn_idx].gpio->pin, btn_config.button[btn_idx].name);
