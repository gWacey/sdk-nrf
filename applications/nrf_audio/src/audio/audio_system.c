...
#define FIFO_OUT_BLOCK_COUNT (USB_OUT_1MS_BLOCKS_NUM_MAX * CONFIG_FIFO_TX_FRAME_COUNT)
...
NET_BUF_POOL_FIXED_DEFINE(audio_q_dec_pool, FIFO_DEC_POOL_BLK_COUNT, PCM_NUM_BYTES_MULTI_CHAN,
			  sizeof(struct audio_metadata), NULL);
NET_BUF_POOL_FIXED_DEFINE(audio_q_out_pool, FIFO_OUT_BLOCK_COUNT, USB_OUT_BLOCK_MULTI_CHAN_1MS_SIZE,
			  sizeof(struct audio_metadata), NULL);
...
int audio_system_decode(struct net_buf *audio_frame_in)
{
	...
	*meta_out = decoder_meta;
	...
	while (k_msgq_num_free_get(&audio_q_out) < USB_OUT_1MS_BLOCKS_NUM_MAX) {
	...
	size_t usb_out_1ms_frame_size =
		USB_OUT_BLOCK_1MS_MONO_SIZE * audio_metadata_num_loc_get(meta_out);
	...
	if (usb_out_spillover != NULL) {
		...
		*usb_spill_meta = *meta_out;
		usb_spill_meta->bytes_per_location = USB_OUT_BLOCK_1MS_MONO_SIZE;
	...
	while (audio_frame_out->len >= usb_out_1ms_frame_size) {
		...
		*usb_block_meta = *meta_out;
		usb_block_meta->bytes_per_location = USB_OUT_BLOCK_1MS_MONO_SIZE;
