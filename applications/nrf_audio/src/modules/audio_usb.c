...
K_MEM_SLAB_DEFINE_STATIC(usb_out_slab,
			 ROUND_UP(USB_OUT_BLOCK_MULTI_CHAN_1MS_SIZE, UDC_BUF_GRANULARITY),
			 USB_BLOCKS, UDC_BUF_ALIGN);
K_MEM_SLAB_DEFINE_STATIC(usb_in_slab, ROUND_UP(USB_IN_BLOCK_MULTI_CHAN_SIZE, UDC_BUF_GRANULARITY),
			 USB_BLOCKS, UDC_BUF_ALIGN);
...
NET_BUF_POOL_FIXED_DEFINE(pool_in, USB_BLOCKS,
			  (USB_IN_BLOCK_MULTI_CHAN_SIZE * CONFIG_FIFO_FRAME_SPLIT_NUM / 2),
...
struct audio_metadata usb_in_meta = {.data_coding = PCM,
				     ...
				     .bytes_per_location = USB_IN_BLOCK_MULTI_CHAN_SIZE / 2,
				     .interleaved = true,
...
static void usb_send_cb(const struct device *dev, void *user_data)
{
	...
	bool out_ready = (audio_q_out != NULL && terminal_headset_in_enabled && playing_state &&
			  local_host_in);
	...
	ret = k_msgq_get(audio_q_out, &usb_data_out, K_NO_WAIT);
	if (unlikely(ret)) {
			     ...
			     void *user_data)
{
	...
	int ret;
	void *usb_in_buf = NULL;

	if (terminal == TERMINAL_ID_HEADSET_OUT || terminal == TERMINAL_ID_HEADPHONES_OUT) {
		ret = k_mem_slab_alloc(&usb_in_slab, &usb_in_buf, K_NO_WAIT);
	}
			 ...
			 void *user_data)
{
	...
	/* Size validation */
	if (unlikely(size != USB_IN_BLOCK_MULTI_CHAN_SIZE)) {
		LOG_WRN("Incorrect buffer size: %d (%u)", size, USB_IN_BLOCK_MULTI_CHAN_SIZE);
		k_mem_slab_free(&usb_in_slab, buf);
		return;
	}
	...
	if (frame_current == NULL) {
		if (frame_spillover != NULL) {
			...
			if (unlikely(k_msgq_num_free_get(audio_q_in) == 0 ||
	...
	if (blocks_in_frame_current >= CONFIG_FIFO_FRAME_SPLIT_NUM) {
		...
		ret = k_msgq_put(audio_q_in, (void *)&frame_current, K_NO_WAIT);
		if (ret) {
...
int audio_usb_start(struct k_msgq *audio_q_out_in, struct k_msgq *audio_q_in_in)
{
	if (audio_usbd == NULL) {
	...
	if (audio_q_out_in == NULL || audio_q_in_in == NULL) {
		return -EINVAL;
	}

	audio_q_out = audio_q_out_in;
	audio_q_in = audio_q_in_in;
