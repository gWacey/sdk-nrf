...
"""
Script to build and program the nRF Audio project to multiple devices
"""
...
NRF_AUDIO_FOLDER = (BUILDPROG_FOLDER / "../..").resolve()
...
TARGET_AUDIO_FOLDER = NRF_AUDIO_FOLDER
TARGET_AUDIO_BUILD_FOLDER = TARGET_AUDIO_FOLDER / "tools/build"

UNICAST_SERVER_OVERLAY = NRF_AUDIO_FOLDER / "unicast_server/overlay-unicast_server.conf"
...
ALT_BROADCAST_NAME_SUFFIX = "_ALT"
MAX_USER_NAME_LEN = 248 -len(ALT_BROADCAST_NAME_SUFFIX) -len('\0')
...
def __main():
    parser = argparse.ArgumentParser(
        ...
        description=(
            "This script builds and programs the nRF Audio project on Windows and Linux"
        ),
    ...
    parser.add_argument(
        ...
        help=(
            "nRF Audio on the application core can be "
    ...
    parser.add_argument(
        ...
        help="Use custom Bluetooth device and/or broadcast name."
         " NOTE: Updating this requires a pristine build."
         " NOTE: Convenience option,"
         " the correct way is to set the name in the project configuration files.",
    )
    parser.add_argument(
        "-t",
        "--transport",
