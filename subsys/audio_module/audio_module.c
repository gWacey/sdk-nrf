...
LOG_MODULE_REGISTER(audio_module, 4); /* CONFIG_AUDIO_MODULE_LOG_LEVEL); */
			    ...
			    struct audio_data *audio_data_rx, k_timeout_t timeout)
{
	...
	ret = data_fifo_pointer_last_filled_get(handle_tx->thread.msg_tx, (void **)&msg_rx,
						&msg_rx_size, timeout);
	if (ret) {
	...
	LOG_DBG("Received message from module %s TX queue", handle_tx->name);
	...
	LOG_DBG("Release internal buffer for module %s", handle_tx->name);
	...
	LOG_DBG("Release RX message for module %s", handle_tx->name);
