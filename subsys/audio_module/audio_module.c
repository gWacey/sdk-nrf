		      ...
		      struct audio_module_handle *handle)
{
	...
	if (handle->thread.msg_rx != NULL && !data_fifo_state(handle->thread.msg_rx)) {
		ret = data_fifo_init(handle->thread.msg_rx);
		if (ret) {
			LOG_ERR("Failed to initialize the RX FIFO for module %s, ret %d",
				handle->name, ret);
	...
	if (handle->thread.msg_tx != NULL && !data_fifo_state(handle->thread.msg_tx)) {
		ret = data_fifo_init(handle->thread.msg_tx);
		if (ret) {
			LOG_ERR("Failed to initialize the TX FIFO for module %s, ret %d",
				handle->name, ret);
...
int audio_module_close(struct audio_module_handle *handle)
{
	...
	if (handle->thread.msg_rx != NULL && data_fifo_state(handle->thread.msg_rx)) {
		data_fifo_uninit(handle->thread.msg_rx);
	}

	if (handle->thread.msg_tx != NULL && data_fifo_state(handle->thread.msg_tx)) {
		data_fifo_uninit(handle->thread.msg_tx);
	}
			 ...
			 k_timeout_t timeout)
{
	...
	if (audio_data->data_size != 0) {
		if (msg_rx->audio_data.data_size > audio_data->data_size) {
			...
			memcpy(&audio_data->meta, &msg_rx->audio_data.meta,
			    ...
			    struct audio_data *audio_data_rx, k_timeout_t timeout)
{
	...
	if (msg_rx->audio_data.data_size != 0) {
		if (msg_rx->audio_data.data_size > audio_data_rx->data_size) {
			...
			ret = -ECANCELED;
		} else if (audio_data_rx->data == NULL) {
			LOG_WRN("Data pointer to buffer is NULL");
		} else {
