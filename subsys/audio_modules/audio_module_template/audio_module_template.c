...
static int audio_module_template_flush(struct audio_module_handle_private *handle)
{
	struct audio_module_handle *hdl = (struct audio_module_handle *)handle;

	/* Perform any other functions to flush the module. */

	LOG_DBG("Flush the %s module", hdl->name);
...
const struct audio_module_functions audio_module_template_functions = {
	...
	/**
	 * @brief Flush a module processing data.
	 */
	.flush = audio_module_template_flush,
