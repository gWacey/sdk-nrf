			     ...
			     struct audio_module_configuration const *const configuration)
{
	...
	for (uint8_t i = 0; i < number_channels; i++) {
		...
		if (ret) {
			LOG_ERR("LC3 encoder channel %d failed to initialise for module %s: %d", i,
				hdl->name, ret);
			return ret;
		}
				   ...
				   struct audio_data *audio_data_out)
{
	...
	coded_out = (uint8_t *)audio_data_out->data;
	coded_out_size = 0;
	...
	for (uint8_t chan = 0; chan < number_channels; chan++) {
		...
		if (ctx->config.interleaved) {
			...
			data_in_size = audio_data_in->data_size;
		}

		LC3EncodeInput_t LC3EncodeInput = {
			.PCMData = data_in, .PCMDataLength = data_in_size, .bytesRead = 0};
