...
NET_BUF_POOL_FIXED_DEFINE(pool_in, CONTIN_TEST_POOL_CONTIN_NUM, CONTIN_TEST_POOL_CONTIN_SIZE,
			  (sizeof(struct audio_metadata)), NULL);
NET_BUF_POOL_FIXED_DEFINE(pool_out, CONTIN_TEST_POOL_FINITE_NUM, CONTIN_TEST_POOL_FINITE_SIZE,
			  sizeof(struct audio_metadata), NULL);

static void allocate_audio_bufs(struct net_buf **pcm_out, struct net_buf **pcm_in)
{
	*pcm_out = net_buf_alloc(&pool_out, K_NO_WAIT);
	*pcm_in = net_buf_alloc(&pool_in, K_NO_WAIT);
}

static void deallocate_audio_bufs(struct net_buf *pcm_out, struct net_buf *pcm_in)
{
	net_buf_unref(pcm_out);
	net_buf_unref(pcm_in);
}
