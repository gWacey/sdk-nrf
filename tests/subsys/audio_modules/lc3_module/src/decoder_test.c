...
enum test_module_id {
	TEST_MODULE_LC3_DECODER = 1,
	TEST_MODULE_LC3_ENCODER,
	TEST_MODULES_NUM
};

enum test_lc3_decoder_module_id {
	TEST_MODULE_ID_DECODER_1 = 1,
	TEST_MODULE_ID_DECODER_2,
	TEST_MODULE_ID_DECODER_NUM
};

struct audio_module_handle handle[CONFIG_DECODER_MODULES_NUM + CONFIG_ENCODER_MODULES_NUM];
...
DATA_FIFO_DEFINE(lc3_dec1_fifo_tx, CONFIG_DECODER_MSG_QUEUE_SIZE,
		 sizeof(struct audio_module_message));
DATA_FIFO_DEFINE(lc3_dec1_fifo_rx, CONFIG_DECODER_MSG_QUEUE_SIZE,
		 sizeof(struct audio_module_message));

K_THREAD_STACK_DEFINE(lc3_dec2_thread_stack, CONFIG_LC3_DECODER_STACK_SIZE);
DATA_FIFO_DEFINE(lc3_dec2_fifo_tx, CONFIG_DECODER_MSG_QUEUE_SIZE,
		 sizeof(struct audio_module_message));
DATA_FIFO_DEFINE(lc3_dec2_fifo_rx, CONFIG_DECODER_MSG_QUEUE_SIZE,
		 sizeof(struct audio_module_message));
...
static void test_lc3_decoder_mono_init(void)
{
	...
	test_decoder_config.bitrate_bps = CONFIG_SW_LC3_BITRATE_MAX;
...
ZTEST(suite_lc3_decoder_functional, test_lc3_decode_stereo_zint)
{
	int ret;
	int dec_call_count = 0;
	uint8_t data_in[TEST_ENC_STEREO_BUF_SIZE];
