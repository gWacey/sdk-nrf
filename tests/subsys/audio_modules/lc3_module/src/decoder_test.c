...
static struct audio_module_handle handle[TEST_LC3_DECODER_MODULES_NUM];
static struct lc3_decoder_context decoder_ctx[TEST_LC3_DECODER_MODULES_NUM];
...
static struct data_fifo *lc3_dec_msg_fifo_tx_array[] = {&lc3_dec_msg_fifo_tx,
							&lc3_dec_msg_fifo_tx1};
static struct data_fifo *lc3_dec_msg_fifo_rx_array[] = {&lc3_dec_msg_fifo_rx,
							&lc3_dec_msg_fifo_rx1};
...
static struct audio_data audio_data = {.meta = {.data_coding = LC3,
...
static void test_lc3_decoder_mono_multi_init(uint32_t locations)
{
	...
	for (int i = 0; i < number_channels; i++) {
		...
		test_decoder_param.thread.msg_rx = lc3_dec_msg_fifo_rx_array[i];
		test_decoder_param.thread.msg_tx = lc3_dec_msg_fifo_tx_array[i];
...
static void test_lc3_decoder_multi_init(bool pcm_format, uint32_t locations)
{
	...
	test_decoder_param.thread.msg_rx = lc3_dec_msg_fifo_rx_array[0];
	test_decoder_param.thread.msg_tx = lc3_dec_msg_fifo_tx_array[0];

	ret = audio_module_open(
		&test_decoder_param, (struct audio_module_configuration *)&test_decoder_config,
		"Dec Multi-channel", (struct audio_module_context *)&decoder_ctx[0], &handle[0]);
	zassert_equal(ret, 0, "Decoder left module open did not return zero");
...
ZTEST(suite_lc3_decoder_functional, test_lc3_decode_mono)
{
	...
	ref_in = (uint8_t *)&wav_mono[0];
	ref_in_size = sizeof(wav_mono);
	ref_in_read_size = TEST_DEC_MONO_BUF_SIZE;
	...
	do {
		...
		dec_call_count += 1;
		  ...
		  dec_call_count < (WAV_MONO_SIZE / TEST_DEC_MONO_BUF_SAMPLES)));
...
ZTEST(suite_lc3_decoder_functional, test_lc3_decode_dual_mono)
{
	...
	do {
		...
		dec_call_count += 1;
		  ...
		  (dec_call_count >> 1) < (WAV_MONO_SIZE / TEST_DEC_MONO_BUF_SAMPLES)));
