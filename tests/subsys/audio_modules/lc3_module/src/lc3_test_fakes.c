...
uint8_t *ref_in;
int ref_in_size;
int ref_in_read_size;

static int enc_count;
static int dec_count;

struct lc3_session dec_session_ctx[TEST_DEC_SESSIONS_NUM];
struct lc3_session enc_session_ctx[TEST_ENC_SESSIONS_NUM];
				     ...
				     uint8_t *buffer, uint32_t *bufferSize)
{
	...
	for (int i = 0; i < TEST_ENC_SESSIONS_NUM; i++) {
		enc_session_ctx[i].session_init = false;
		enc_session_ctx[i].ref_data = NULL;
		enc_session_ctx[i].ref_read_size = 0;
		enc_session_ctx[i].ref_remaining = 0;
	}
						       ...
						       uint16_t *bufferSize, int32_t *result)
{
	...
	ARG_UNUSED(buffer);
	ARG_UNUSED(bufferSize);
	struct lc3_session *session = (struct lc3_session *)&enc_session_ctx[enc_count];

	session->session_init = true;
	session->ref_data = ref_in;
	session->ref_read_size = ref_in_read_size;
	session->ref_remaining = ref_in_size;
					    ...
					    LC3EncodeOutput_t *encodeOutput)
{
	...
	struct lc3_session *session = (struct lc3_session *)encodeHandle;
	uint8_t *data_out = (uint8_t *)encodeOutput->outputData;
	...
	encodeOutput->bytesWritten = size;
						       ...
						       uint16_t *bufferSize, int32_t *result)
{
	...
	ARG_UNUSED(buffer);
	ARG_UNUSED(bufferSize);
	struct lc3_session *session = (struct lc3_session *)&dec_session_ctx[dec_count];
					    ...
					    LC3DecodeOutput_t *decodeOutput)
{
	...
	struct lc3_session *session = (struct lc3_session *)decodeHandle;
