...
#define RECONFIG_TEXT "Tone reconfig"
...
static struct audio_module_tone_generator_configuration configuration = {
	.sample_rate_hz = 48000, .bits_per_sample = 16, .carried_bits_pr_sample = 16,
	.amplitude = , .interleaved = false};
static struct audio_module_tone_generator_configuration reconfiguration = {
	.sample_rate_hz = 16000, .bits_per_sample = 16, .carried_bits_pr_sample = 32,
	.amplitude = , .interleaved = false};
static struct audio_module_tone_generator_configuration return_configuration;

static struct audio_module_tone_generator_context context = {0};
...
ZTEST(suite_audio_module_tone_generator, test_module_tone_generator)
{
	...
	AUDIO_MODULE_PARAMETERS(mod_parameters, audio_module_tone_generator_description, 
		mod_temp_stack, TONE_GENERATOR_STACK_SIZE, TONE_GENERATOR_THREAD_PRIO, NULL, 
...

	{
		struct audio_module_tone_generator_context *ctx =
			(struct audio_module_tone_generator_context *)handle[0].context;
		struct audio_module_tone_generator_configuration *test_config = &ctx->config;
		...
		zassert_equal(test_config->bits_per_sample, configuration.bits_per_sample,
			      "Failed to configure module in the open function");
		zassert_equal(test_config->carried_bits_pr_sample, configuration.carried_bits_pr_sample,
				  "Failed to configure module in the open function");
	...
	zassert_equal_ptr(handle[0].description->name, audio_module_tone_generator_description->name,
			  "Failed open for names, base name should be %s, but is %s",
			  audio_module_tone_generator_description->name, handle[0].description->name);
	...
	zassert_mem_equal(base_name, audio_module_tone_generator_description->name,
			  strlen(audio_module_tone_generator_description->name),
			  "Failed get names, base name should be %s, but is %s",
			  audio_module_tone_generator_description->name, base_name);
	...
	for (i = 0; i < TEST_AUDIO_DATA_ITEMS_NUM; i++) {
		...
		zassert_mem_equal(ref_meta, &audio_data_rx.meta,
		...
		if (i == TEST_AUDIO_DATA_ITEMS_NUM / 2) {
			...
			zassert_equal(
				...
				ret);
			zassert_equal(reconfiguration.sample_rate_hz, reconfiguration.sample_rate_hz,
				      "Failed to reconfigure module");
			zassert_equal(reconfiguration.bits_per_sample, reconfiguration.bits_per_sample,
				      "Failed to reconfigure module");
			zassert_equal(reconfiguration.carried_bits_pr_sample, reconfiguration.carried_bits_pr_sample,
				      "Failed to reconfigure module");
